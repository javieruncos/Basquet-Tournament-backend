...
router.post('/clubes',verifyTokenAuth,verifyAdmin,upload.single('logo'),crearClubes)
router.get('/clubes',obtenerClubes)
router.get('/clubes/:id',upload.single('logo'),obtenerClubesPorId)
router.put('/clubes/:id',verifyTokenAuth,verifyAdmin,upload.single('logo'),actualizarClubes)
